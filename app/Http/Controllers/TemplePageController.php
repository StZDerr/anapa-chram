...
class TemplePageController extends Controller
{
    ...
    public function kupelOlgiEdit()
    {
        $page = TemplePage::getByKey('kupel-olgi') ?? new TemplePage(['page_key' => 'kupel-olgi']);
     ...
     * Update the temple page content.
     */
    public function kupelOlgiUpdate(Request $request)
    {
        $data = $request->validate([
            'title' => 'nullable|string|max:500',
            'about_text' => 'nullable|string',
        ...
        $page = TemplePage::where('page_key', 'kupel-olgi')->first();
        ...
        return redirect()->route('admin.temple.kupelOlgi.edit')
            ->with('success', 'Страница Купели Ольги успешно обновлена!');
    }

    public function derzhavnayaIkonaEdit()
    {
        $page = TemplePage::getByKey('derzhavnaya-ikona') ?? new TemplePage(['page_key' => 'derzhavnaya-ikona']);
     ...
     * Update the temple page content.
     */
    public function derzhavnayaIkonaUpdate(Request $request)
    {
        $data = $request->validate([
            'title' => 'nullable|string|max:500',
            'about_text' => 'nullable|string',
        ...
        $page = TemplePage::where('page_key', 'derzhavnaya-ikona')->first();
        ...
        return redirect()->route('admin.temple.derzhavnayaIkona.edit')
            ->with('success', 'Страница Державной иконы Божией Матери успешно обновлена!');
    }
}
